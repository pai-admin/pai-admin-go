...
func (ru redisUtil) Set(key string, value interface{}, timeSec int) bool {
	err := core.Redis.Set(context.Background(), key, value, time.Duration(timeSec)*time.Second).Err()
	if err != nil {
...
func (ru redisUtil) Get(key string) string {
	res, err := core.Redis.Get(context.Background(), key).Result()
	if err != nil {
...
func (ru redisUtil) SSet(key string, values ...interface{}) bool {
	err := core.Redis.SAdd(context.Background(), key, values...).Err()
	if err != nil {
...
func (ru redisUtil) SGet(key string) []string {
	res, err := core.Redis.SMembers(context.Background(), key).Result()
	if err != nil {
...
func (ru redisUtil) HMSet(key string, mapping map[string]string, timeSec int) bool {
	err := core.Redis.HSet(context.Background(), key, mapping).Err()
	if err != nil {
...
func (ru redisUtil) HGet(key string, field string) string {
	res, err := core.Redis.HGet(context.Background(), key, field).Result()
	if err != nil {
...
func (ru redisUtil) HExists(key string, field string) bool {
	res, err := core.Redis.HExists(context.Background(), key, field).Result()
	if err != nil {
...
func (ru redisUtil) HDel(key string, fields ...string) bool {
	err := core.Redis.HDel(context.Background(), key, fields...).Err()
	if err != nil {
...
func (ru redisUtil) Expire(key string, timeSec int) bool {
	err := core.Redis.Expire(context.Background(), key, time.Duration(timeSec)*time.Second).Err()
	if err != nil {
...
func (ru redisUtil) TTL(key string) int {
	td, err := core.Redis.TTL(context.Background(), key).Result()
	if err != nil {
